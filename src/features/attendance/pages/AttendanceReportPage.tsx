// src/features/attendance/pages/AttendanceReportPage.tsx
import { useState } from "react";
import { useQuery } from "@tanstack/react-query";
import { format } from "date-fns";
import { es } from "date-fns/locale";
import { FileText, Filter, Download, Calendar, CheckCircle, XCircle, Search } from "lucide-react";
...
const AttendanceReportPage = ({ initialMode = "general" }: AttendanceReportPageProps) => {
  ...
  const handleDownload = async () => {
    if (!reportData || reportData.attendances.length === 0) {
      toast({
        title: "No hay datos",
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Genera y descarga reportes detallados de asistencia.</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-6">
        ...
        <div className="mt-4 flex justify-end gap-3">
          ...
          {reportData && reportData.attendances.length > 0 && (
            <Button onClick={handleDownload} disabled={isDownloadingPDF}>
              <Download className="w-4 h-4 mr-2" />
              {isDownloadingPDF ? "Descargando..." : "Exportar PDF"}
            </Button>
          )}
        </div>
      </div>
